...
\begin{document}
	...
	\begin{titlepage}
		...
		{\Huge \textbf{Exploring Lombardy Through Data}}\\[0.4cm]
	...
	This project analyzes air quality, weather, traffic, and tourism across Lombardy in 2023. The data comes from ARPA Lombardia, the Municipality of Milan, and ISTAT.

	The year 2023 was chosen because the traffic dataset only covers that period. Restricting all sources to the same window keeps things consistent and cuts down on processing time.

	Four data domains feed into the analysis:
	\begin{itemize}
		\item \textbf{Air and Weather:} hourly and daily sensor readings for pollutants and meteorological variables across Lombardy, with station and sensor registry metadata (ARPA Lombardia).
	...
	\begin{itemize}
		\item A Data Warehouse is designed and filled through an ETL pipeline that extracts, cleans, and loads everything into a PostgreSQL database.
		\item Then, \textbf{Advanced SQL}, \textbf{Association Rules}, and \textbf{Machine Learning} are applied to answer the business questions laid out in the next section.
	\end{itemize}
	...
	\textbf{How can vehicle access data be analyzed to understand fleet composition, temporal patterns, and traffic behavior?}
	\begin{itemize}
		\item What is the composition of the fleet of vehicles accesses and how is it distributed through the year?
	...
	Hourly or daily air pollution readings from monitoring stations across Lombardy. Each station hosts one or more pollutant sensors.
	...
	Monthly tourism flows per municipality and accommodation type across Lombardy.
	...
	Municipality boundaries in GeoJSON format.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{./images/WeatherAirSampleDFM.png}
	\end{figure}
	...
	Descriptive attributes for every Lombardy municipality: name, urbanization level, area, altimetric zone, coastal and island flags, and a pointer to geographic coordinates in \texttt{location\_dt}.
	\begin{table}[H]
		\centering
		\begin{tabular}{lll}
			\hline
	...
	Annual resident population per municipality. Links a municipality to a reference year via \texttt{date\_dt}, so that population-normalized indicators (e.g.\ per-capita pollution, tourism pressure ratios) can be computed.
	\begin{table}[H]
		\centering
		\begin{tabular}{lll}
			\hline
...
class Source(ABC):
	...
	Raw CSVs are loaded through typed \texttt{Source} subclasses. The resulting DataFrame goes straight to the transformation step nothing is persisted in between.
	...
	\begin{itemize}
		...
		\item \textbf{Temporal aggregation:} rolling data up to the target granularity. Sensor samples, for instance, are aggregated to daily level with mean, min, max, and standard deviation. Again, not every table needed it.
	...
	Each analysis targets one of the business questions from Section 2, using a mix of Advanced SQL, Association Rule Mining, and Machine Learning.
	...
	Statistics were computed per pollutant and urbanization level and compared to the Lombardy-wide aggregate.
	...
	The high \textbf{O$_3$} in rural areas is no surprise. NO from urban traffic scavenges ozone before it builds up, so dense areas end up with lower O$_3$ than their rural surroundings.

	\textbf{PM$_{10}$} and \textbf{PM$_{2.5}$} show almost no gradient across urbanization levels. Concentrations even peak in \textbf{Intermediate} areas. Particulate matter is a \textbf{regional problem}, not an urban one.

	\textbf{PM$_{2.5}$} stands apart. It is the only pollutant whose annual mean exceeds the limit in \emph{all} three urbanization categories, uniformly. That alone sets it apart from the rest and argues for region-wide structural measures rather than city-level fixes.

	No single urbanization level dominates across all pollutants. Dense areas lead for \textbf{NO$_2$}. Intermediate zones are worst for \textbf{PM$_{10}$} and \textbf{PM$_{2.5}$} evidence of non-traffic sources. Rural areas top the chart for \textbf{O$_3$} because of reduced NO titration.
	...
	Seasonality is strong. Exceedances cluster in predictable calendar windows.

	Two cases break the pattern. \textbf{Cinisello Balsamo} registers an NO$_2$ episode in mid-summer, when traffic is typically low. That is unusual and suggests a local non-traffic source. \textbf{Moggio} shows O$_3$ exceedances from May all the way to October far longer than anywhere else. Its altitude, exposure, and low local NO emissions probably explain why ozone persists there well outside the normal summer window.
	...
	\begin{figure}[H]
		...
		\begin{minipage}{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{./images/1_air_quality/cluster_pca.png}
			\caption{2D PCA projection}
		\end{minipage}
		\hfill
		\begin{minipage}{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{./images/1_air_quality/cluster_geo_map.png}
			\caption{Cluster location map}
		\end{minipage}
	\end{figure}

	\textbf{Cluster 1} contains most of the monitored municipalities. \textbf{Cluster 0} picks up a smaller, more polluted subset. \textbf{Cluster 2} consists of a \textbf{single municipality}: its profile is so different from everything else that it ends up alone. It is best read as an outlier, not a representative group.
	...
	\textbf{Cluster 0} is the dirtiest group. Its primary-pollutant concentrations are the highest year-round, and in winter both PM$_{2.5}$ and NO$_2$ regularly breach guideline levels.

	\textbf{Cluster 1} sits near the regional average. \textbf{Cluster 2}, the lone municipality, has readings extreme enough to form its own cluster, confirming it is a statistical outlier.
	...
	The ranking matches the trend charts. \textbf{Cluster 2} has the highest index (0.887), but that is driven by one municipality. \textbf{Cluster 0} comes next at 0.779, the dirtiest multi-municipality group. \textbf{Cluster 1} is well below at 0.471.
	...
	Comparing the baseline shares to top-5 appearances tells a consistent story. Dense municipalities are \textbf{under-represented} relative to their share of the monitoring network; Intermediate and Rural areas are \textbf{over-represented} for both pollutants.

	The gap is widest for PM$_{10}$: Intermediate municipalities make up 68.3\% of appearances versus a 56\% baseline, while Dense areas drop to 13.3\% against 23\%. One plausible reading is that urban traffic restrictions and centralized heating regulations are doing their job in large cities, but peripheral industrial and agricultural zones remain outside current policy reach.

	For PM$_{2.5}$, Rural municipalities appear in 28.3\% of slots versus a 21\% baseline. That pattern fits biomass burning and agricultural emissions sources that urban air quality frameworks barely touch. 
	
	Particulate pollution is not just a city problem, and abatement strategies need to look beyond municipal boundaries.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.5\textwidth]{./images/2_weather_pollution/correlation_heatmap.png}
	\end{figure}
	
	Two opposite regimes are visible. \textbf{O$_3$} correlates strongly with radiation and temperature (0.76 and 0.75), no surprise given its photochemical origin. The primary pollutants (NO$_2$, PM$_{10}$, PM$_{2.5}$) go the other way, showing negative correlations with the same variables: they accumulate under cold, stagnant, low-radiation conditions.

	Two less obvious results. \textbf{Humidity} is the only variable that flips sign across pollutant groups: positive for particulates (up to 0.33 for PM$_{2.5}$), negative for O$_3$ ($-0.51$). At daily resolution, humidity apparently helps PM accumulate rather than washing it out. \textbf{Rain}, on the other hand, is nearly uncorrelated with everything (0.00 to $-0.16$). Precipitation by itself is a poor predictor of air quality.
	...
	Apriori was run on the resulting binary matrix with minimum support 0.5\% and minimum confidence 20\%; only rules with lift $\geq$ 1.20 and a weather-only antecedent mapping to a pollutant-only consequent were kept.
	...
	Three meteorological regimes emerge from the rules, each with its own pollution signature.

	The first and most common is \textbf{atmospheric stagnation}: low wind, little rain, moderate temperature. It appears in most top-ranked rules and co-occurs with \texttt{PM10\_HIGH} and \texttt{O3\_MID}, hitting a lift of 7.37. The consistency across different antecedent combinations (lifts 6.05-7.37) makes stagnation the single best meteorological predictor of pollution peaks.

	The second is a \textbf{decoupling regime} where high radiation coincides with rain and wind. Under these conditions NO$_2$ and PM$_{10}$ drop to low levels (wet scavenging clears them out), but O$_3$ stays high (confidence up to 46.7\%) because photochemistry keeps running as long as there is enough sunlight.

	The third regime combines \texttt{HUMIDITY\_MID}, \texttt{RADIATION\_MID}, and \texttt{WIND\_LOW}. Here, NO$_2$, O$_3$, and PM$_{10}$ all accumulate at the same time (lift 6.43-6.81). This multi-pollutant build-up is typical of transitional seasons and is the hardest scenario to address with single-pollutant regulations.
	
	\newpage
	
	\subsection{How can vehicle access data be analyzed to understand fleet composition, temporal patterns, and traffic behavior?}
	
	\subsubsection{What is the composition of the fleet of vehicles accesses and how is it distributed through the year?}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{./images/3_traffic/total_accesses_monthly.png}
	\end{figure}
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{./images/3_traffic/total_accesses_daily.png}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\begin{minipage}{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{./images/3_traffic/fleet_composition_by_fuel_monthly.png}
			\caption{Monthly fleet composition by fuel type.}
		\end{minipage}
		\hfill
		\begin{minipage}{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{./images/3_traffic/total_accesses_by_fuel.png}
			\caption{Total accesses by fuel type.}
		\end{minipage}
	\end{figure}
	
	Combustion vehicles still dominate. Benzina and Diesel together make up about 58\% of accesses, but \textbf{Ibrido Benzina Elettrico} already accounts for 19\%, pushing total low-emission share to roughly 23\%. 
	...
	\begin{figure}[H]
		...
		\begin{minipage}{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{./images/3_traffic/fleet_composition_by_euro_monthly.png}
			\caption{Monthly fleet composition by Euro standard.}
		\end{minipage}
		\hfill
		\begin{minipage}{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{./images/3_traffic/total_accesses_by_euro.png}
	...
	A \textbf{Random Forest Regressor} was trained to predict daily accesses. Features: lagged traffic (1, 2, 3, and 7 days back), a 7-day rolling mean and standard deviation, day of week, month, a weekend flag, a holiday indicator (\texttt{is\_holiday}) covering Italian national holidays plus Saint Ambrose Day on 7 December, and a bridge-day indicator (\texttt{is\_bridge}) for workdays squeezed between a holiday and a weekend.
	...
	R$^2$ = 0.724. The autoregressive and calendar features capture about 72\% of daily variance. The missing 28\% is presumably weather, big public events, and transport strikes, none of which appear in the feature set.

	The residuals show a clear pattern around \textbf{holidays and bridge days}. The model over-predicts because its lag features carry forward normal working-day volumes and cannot anticipate the sudden drop. Binary \texttt{is\_holiday} and \texttt{is\_bridge} flags from the Italian holiday calendar (subdivision Milano) were added to compensate.
	The mirror problem shows up on post-holiday return days in April and September: traffic rebounds sharply, producing the largest under-predictions. The bridge indicator partially handles these transitions too.

	\subsubsection{Which days show anomalous traffic volumes?}
	
	An \textbf{Isolation Forest} (200 estimators, 5\% contamination) was fitted on daily traffic totals.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.4\textwidth]{./images/3_traffic/anomaly_by_month.png}
		\caption{Monthly distribution of detected anomalies.}
	\end{figure}
	
	\textbf{18 days} were flagged, splitting neatly into two groups. 
	The \textbf{low-traffic anomalies} cluster around Ferragosto: Italy's main summer shutdown pushes volumes well below baseline.
	The \textbf{high-traffic anomalies} land on predictable calendar transitions: the days around Easter, the end-of-summer rebound, and Fridays before long weekends. 
	...
	Both traffic and pollutant values were binned into three levels (LOW, MID, HIGH) using global tertiles over the Milan daily series.
	...
	The \texttt{TRAFFIC\_LOW} rules dominate (lift up to \textbf{1.89}), but they are picking up \textbf{seasonal confounding}. Low traffic and \texttt{NO2\_LOW + O3\_HIGH} co-occur because August Ferragosto kills commuter traffic at exactly the time when summer meteorology pushes ozone up. That is a calendar effect, not a causal link.
	
	The \texttt{TRAFFIC\_MID} rules (lift \textbf{1.42-1.45}) pair intermediate volumes with \texttt{NO2\_HIGH + O3\_LOW + PM25\_HIGH}. This matches autumn and winter workdays: \textbf{atmospheric stagnation} traps pollutants near the surface regardless of how many cars enter Area C.
	
	The \texttt{TRAFFIC\_HIGH} bin yields just one rule (\texttt{NO2\_MID + PM25\_MID}, lift \textbf{1.71}). Peak traffic volumes do not produce any distinctive pollution signature beyond what the season already explains.

	The overall maximum lift is \textbf{1.89}, low by association-rule standards. Two structural limitations apply. First, \textbf{geography}: traffic and pollution data are both restricted to Milan, so nothing here generalizes to the rest of Lombardy. Second, \textbf{instrument mismatch}: Area C monitors a single corridor into the city centre and captures only a fraction of total urban mobility, making it a weak proxy for actual emission intensity. In practice, traffic volume acts as a \textbf{calendar proxy}: it encodes time-of-year and holiday patterns, not emission load. A more informative analysis would need region-wide counts from motorways, provincial roads, and other cities.
	...
	The \texttt{TOURISM\_LOW} rules take up 14 of the top 20 slots (lift up to \textbf{2.65}). \textbf{Winter stagnation} is doing the work here: low-tourism months line up with the classic Po Valley pollution fingerprint: \texttt{NO2\_HIGH}, \texttt{PM10\_HIGH}, \texttt{PM25\_HIGH}, \texttt{O3\_LOW}. The single strongest association links \texttt{TOURISM\_LOW} to \texttt{NO2\_HIGH} at \textbf{66.7\%} confidence. That is \textbf{thermal inversion} in winter, not some effect of fewer tourists.
	
	The \texttt{TOURISM\_HIGH} rules (6 rules, lift up to \textbf{1.72}) tie peak visitor months to \texttt{NO2\_LOW} and \texttt{O3\_HIGH}, exactly what you would expect from high radiation and better ventilation in summer. Nothing here points to tourism itself improving air quality.

	The higher lift for tourism (\textbf{2.65}) compared to traffic does not mean tourism has a stronger physical effect. At monthly granularity, daily noise disappears, leaving little more than the seasonal weather pattern to correlate with.
	
	Tourism intensity works as a \textbf{calendar proxy}. No rule connects high visitor numbers to pollution levels above what the season already produces. Municipality-scale air quality is driven by large-scale meteorology, not by visitor counts.

	\newpage
	\section{Generative artificial intelligence tools used}

	AI tools like GitHub Copilot, Google Gemini, and ChatGPT were used during the project for:
	\begin{itemize}
		\item support writing and debugging Python code, especially when working with unfamiliar libraries
		\item dataset discovery via deep-search features during the research phase
		\item drafting documentation, checking text, fixing grammar, and translating to English
	\end{itemize}
